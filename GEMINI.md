...
- 分支命名: `gemini/{task-name}`（Main Agent 从 `develop` 创建并 checkout 后交给你）
...
- `source.reporterType`: "dev"，`source.reporterId`: "gemini-cli"
...
- System prompt injected via `GEMINI_SYSTEM_MD` env var (file path)
- Session resume: `--resume <UUID>`
- Image input: `@./path` temp file reference

...
| Need transparent background (any model) | Pipeline argparse hard-rejects `--background transparent` regardless of `--model` — the choices list is `auto \| opaque` only, so `gpt-image-1.5` (which natively supports transparent BG) cannot be exercised through the pipeline today | Phase 3 follow-up: extend `scripts/image_gen/__main__.py` argparse choices to include `transparent` and route conditionally per `--model`. As a near-term workaround, call the adapter directly: `python adapters/gpt-image-2/invoke.py --model gpt-image-1.5 --background transparent ...` (bypasses the Slice B verify rubric — caller takes on integration responsibility) |

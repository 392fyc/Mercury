...
Codex 日常默认原生自主执行，实施与审查深度按任务风险选择；仅在用户明确调用或已确定的任务方案需要时使用此预设。
...
1. **Issue-first**: 仓库改动前关联 GitHub Issue，PR 引用对应 Issue。
2. **原生自主执行**: Codex 日常直接完成任务，需要独立分工时可委派有明确范围的任务；复杂流程与多阶段预设仅在用户明确调用或已确定的任务方案需要时使用。实施代理默认继承调用方模型与推理设置，其他角色保留各自配置。
3. **按风险验证**: 运行相关检查并检查最终差异；重要行为改动、跨仓库写入、安全或权限变更、代理指令或规则变更须独立审查。双路审查与多阶段链仅在用户明确调用或已确定的任务方案需要时使用。优先使用原生或挂载机制，不重复自研门禁。
4. **审查后交付**: 由任务指定实施者与 Git 交付者；在任务要求交付时提交，受托代理仅在明确分配提交或推送时执行。先完成所需验证和审查，再通过项目 guard 包装提交；所有代码通过具备所需检查和批准的 PR 合入 develop。
5. **中文沟通**: 进度与完成消息使用正常、清楚的简体中文。

...
> **Platform note**: GitHub and CodeRabbit examples use bash syntax so the `.agents` and `.claude` variants stay aligned. The final RPC snippet also shows a PowerShell/Codex alternative for Windows.
## When
- After dev work reaches `implementation_done`, is committed and pushed, and the task has passed `main_review`.
- When the user asks to create, inspect, or merge a PR.
- When CI or CodeRabbit on an open PR requires another bounded fix-review loop.
## Pipeline
...
```
1. **Poll CI checks with scope helpers**
```bash
...
```
If checks fail, run `extract_changed_paths(failure)`, call `is_within_scope(path, allowedWriteScope.codePaths...)` for each path, auto-fix only in-scope `lint` or `type` failures, and report out-of-scope `test` or `build` failures instead of guessing.
...
```
Timeout is 15 minutes total with 60-second polls. Completion requires at least one `APPROVED` review and zero `CHANGES_REQUESTED`; otherwise escalate instead of merging.
...
')
```
Use the explicit first-line parser `/(Potential issue|Nitpick|Verification successful).*?(Critical|Major|Minor|Trivial)?/`. Treat out-of-scope paths or comments without ` ```suggestion` / ` ```diff` content as `skip`, not success.
...
check_unresolved_critical_comments() { [ "$(echo "$ACTIONABLE" | jq -s 'length')" -eq 0 ]; }
...
```
After merge, update Mercury state with a resilient PowerShell example:
```powershell
...
```
If you prefer bash on a non-Windows runner, use the same JSON payload with `curl -s http://localhost:${MERCURY_RPC_PORT:-7654}/rpc ...`.
## Output
```text
## PR Flow Results
PR: #<number> (<url>)
